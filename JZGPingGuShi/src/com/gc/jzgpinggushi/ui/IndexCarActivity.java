...
import com.gc.jzgpinggushi.adapter.ModelCategoryAdapter;
import com.gc.jzgpinggushi.adapter.StyleCategoryAdapter;
...
import com.gc.jzgpinggushi.uitls.ChineseUtil;
import com.gc.jzgpinggushi.uitls.DialogManager;
import com.gc.jzgpinggushi.uitls.MessageUtils;
import com.gc.jzgpinggushi.view.MyLetterListView;
import com.gc.jzgpinggushi.view.MyLetterListView.OnTouchingLetterChangedListener;
		...
		OnScrollListener
{
				...
				if (makeList.getStatus() == SUCCESS)
				{
					...
					if (makes != null)
					{
						...
						for (int i = 0; i < makes.size(); i++)
						{
							...
							// System.out.println("full Spell is ：" +
							// contactSort);
							items.add(map);
						}
						Comparator comp = new Mycomparator();
						Collections.sort(items, comp);
						mIndexCarListAdapter = new ListAdapter(
	...
	private void startMakeListThread()
	{
		new Thread(new Runnable()
		{
			...
			public void run()
			{
				...
				{
					e.printStackTrace();
					MessageUtils.sendMessage(mHandler, R.id.net_error, null);
				}
			}
		}).start();
	}
	...
	private void styleItemClick(int position)
	{
		...
		mCurYear = style.getYear();
		// 查车入口
		startQueryCarActivity();
	}

	private void startQueryCarActivity()
	{
		 ActivityHelp.startActivity(this, QueryCarActivity.class, "styleId",
		 String.valueOf(mCurStyleid));
	}
